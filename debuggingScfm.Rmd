...
parameters <- list(
  ...
  scfmDriver = list(
    targetN = 2000, # default 4000; increase targetN for more robust estimates, longer run-time
    .useCache = globCache,
...
studyAreaCalibration <- st_buffer(studyArea, 20000)

rasterToMatchCalibration <- rast(ext(studyAreaCalibration), res = c(250, 250))
...
rasterToMatch <- crop(rasterToMatchCalibration, studyArea)
...
if (FALSE) {
  ...
  studyAreaCalibration <- st_buffer(studyArea, 1e5)
  
  rasterToMatch <- prepInputsLCC(year = 2005, destinationPath = paths$inputPath,
                                 studyArea = studyArea) |>
    disagg(fact = 2)
  rasterToMatchCalibration <- prepInputsLCC(year = 2005, destinationPath = paths$inputPath,
                                      studyArea = studyAreaCalibration) |>
    disagg(fact = 2)

  studyArea <- st_transform(studyArea, crs = crs(rasterToMatch))
  studyAreaCalibration <- st_transform(studyAreaCalibration, crs = crs(rasterToMatch))
...
fireRegimePolysCalibration <- prepInputsFireRegimePolys(studyArea = studyAreaCalibration, 
                                                  rasterToMatch = rasterToMatchCalibration,
                                                  destinationPath = paths[["inputPath"]],
                                                  type = "FRT")
...
objects <- list(
  ...
  fireRegimePolysCalibration = fireRegimePolysCalibration
)

outSim <- simInitAndSpades(times = list(start = 1, end = 1000), 
                           params = parameters, 
                           modules = modules,
                           objects = objects, 
                           paths = paths)
...
newObjects <- list("fireRegimePolys" = newLandscape, 
                   "flammableMapCalibration" = newFlamAreaLarge,
