defineModule(sim, list(
  ...
  description = paste(
    "Diagnostic plots for scfm. Can be run for a single simulation,",
  ...
  parameters = bindrows(
    defineParameter(
      "mode", "character", "single", NA, NA,
      paste(
        "use 'single' to run part of an scfm simulation (i.e., along with other scfm modules);",
        "use 'multi' to run as part of postprocessing multiple scfm runs."
      )
    ),
    defineParameter(
      "reps", "integer", NA_integer_, 1L, NA_integer_,
      paste("number of replicates/runs per study area when running in 'multi' mode.")
    ),
    defineParameter(
      "simOutPrefix", "character", "mySimOut", NA_character_, NA_character_,
      "saved simList file prefix"
    ),
    defineParameter(
      "simTimes", "numeric", c(NA, NA), NA, NA,
      "Simulation start and end times when running in 'multi' mode."
    ),
    defineParameter(
      ".plots", "character", c("screen", "png"), NA, NA,
  ...
  outputObjects = bindrows(
    createsOutput("scfmSummaryDT", "data.table",
                  desc = paste(
                    "Summary data.table containing diagnostic plot data.",
  ...
  switch(eventType,
         ...
         diagnosticPlotsMulti = {
           ...
           summaryDT <- rbindlist(lapply(allReps, function(r) {
             ...
             fsim <- file.path(
               outputPath(sim), sprintf("rep%02d", r),
