...
- updated objects `fireRegimePolys` and `fireRegimePolysLarge` replace objects `landscapeAttr` and `landscapeAttrLarge`, respectively, which have been removed;
- object `cellsByZone` has been removed, as this attribute is now stored in `fireRegimePolys` and `fireRegimePolysLarge`;
