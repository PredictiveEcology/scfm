...
#    `functionName` is a function's name in your module (e.g., `classifyAgeEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H17/Lab/scfmModules/classifyAge/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("classifyAge")
  ...
  parameters <- list(
    ...
    classifyAge = list(.saveInitialTime = NA)
  )
  ...
  if (exists("classifyAgeEvent1", envir = .GlobalEnv)) {
    simOutput <- classifyAgeEvent1(mySim)
  } else {
    simOutput <- mySim$classifyAgeEvent1(mySim)
  }
  ...
  if (exists("classifyAgeEvent2", envir = .GlobalEnv)) {
    simOutput <- classifyAgeEvent2(mySim)
  } else {
    simOutput <- mySim$classifyAgeEvent2(mySim)
  }
