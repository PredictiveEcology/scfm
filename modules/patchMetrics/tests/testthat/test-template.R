...
#    `functionName` is a function's name in your module (e.g., `patchMetricsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H17/Lab/scfmModules/patchMetrics/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("patchMetrics")
  path <- list(modulePath = "/Users/stevec/Dropbox/Courses/7043H17/Lab/scfmModules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    patchMetrics = list(.saveInitialTime = NA)
  )
  ...
  if (exists("patchMetricsEvent1", envir = .GlobalEnv)) {
    simOutput <- patchMetricsEvent1(mySim)
  } else {
    simOutput <- mySim$patchMetricsEvent1(mySim)
  }
  ...
  if (exists("patchMetricsEvent2", envir = .GlobalEnv)) {
    simOutput <- patchMetricsEvent2(mySim)
  } else {
    simOutput <- mySim$patchMetricsEvent2(mySim)
  }
