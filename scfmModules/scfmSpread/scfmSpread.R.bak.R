...
defineModule(sim, list(
  ...
  parameters = rbind(
    defineParameter("spreadProb","numeric",0.4,0,1,desc="spread module for BEACONs"),
  ...
  inputObjects = data.frame(
    objectName = c("spreadState","flammableMap"),
    objectClass = c("data.table","RasterLayer"),
    sourceURL = "",
    other = NA_character_,
...
doEvent.scfmSpread = function(sim, eventTime, eventType, debug = FALSE) {
  if (eventType == "init") {
    ...
    Plot(sim$burnMap, legendRange=c(0,length(sim$ignitionLoci)))
...
scfmSpreadInit <- function(sim) {
  ...
  if (!("ageMap" %in% names(objs(sim)))){
    ...
    sim$burnMap<-sim$vegMap * 0  # 0 * NA = NA
  }
  
  setColors(sim$burnMap) <- colorRampPalette(c("yellow", "red"))(10)
...
scfmSpreadBurnemup <- function(sim){ #name is a homage to Walters and Hillborne
  ...
  pSpread <- ifelse("scfmPars" %in% names(objs(sim)),
             ...
             params(sim)$scfmSpread$spreadProb
  )
