...
df <- comparePredictions(names(outSim$scfmDriverPars), outSim) %>%
  rbindlist(.)
#Some useful plots
diagnosticPlot1 <- ggplot(df, aes(x = histMeanSize, y = modMeanSize)) +
  geom_point(aes(histMeanSize, modMeanSize)) + 
  labs(y = "modeled mean fire ize", x = "historical mean fire size") +
  theme_minimal() + 
...
#targetFRI temporarily hard-coded - 
diagnosticPlot2 <- ggplot(df[!is.infinite(achievedFRI) & 
...
diagnosticPlot3 <- ggplot(df, aes(x = targetIgnitions, y = achievedIgnitions)) +
  geom_point() +
...
fireRegimePolys <- outSim$fireRegimePolys 
fireRegimePolys$PolyID <- as.factor(fireRegimePolys$PolyID) #for ggplot
fps <- ggplot(fireRegimePolys) + 
...
clearPlot()
grid.arrange(fps,  diagnosticPlot1,  diagnosticPlot2,  diagnosticPlot3, 
             nrow = 2, ncol = 2)
