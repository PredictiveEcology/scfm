...
parameters <- list(
  scfmLandcoverInit = list(
    sliverThreshold = 1e8),  
  #polygons smaller than 100 km2 are merged with closest nonsliver neighbour
  scfmIgnition = list(),
  scfmEscape = list(),
  scfmSpread = list(
...
rasterToMatch <- LandR::prepInputsLCC(studyArea = studyArea, destinationPath = paths$inputPath)
...
#do not pass filename2 or it will cause headaches when vegMap is downloaded due to internal cache calls
...
comparePredictions <- function(polyList, simList) {
  out <- lapply(polyList, FUN = function(x, sim = simList) {
    ...
    fireRegimePoints <- outSim$fireRegimePoints[outSim$fireRegimePoints$PolyID == as.numeric(x),]
