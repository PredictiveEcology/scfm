...
####RasterToMatch:
#if you supply studyArea you should supply rtm to make sure the crs are identical. A default rasterToMatch is provided, however some Spades-Ecocystem modules do not require studyArea and rtm to have identical crs. This is a requirement for scfm  due to overlay fxns. 
...
comparePredictions <- function(polyList, simList) {
  out <- lapply(polyList, FUN = function(x, sim = simList) {
    ...
    mod_MAAB <- sum(burnSum$areaBurned)/(landscapeAttr$burnyArea * (times$end - times$start))
