...
In this case, `scfmLandcoverInit` will calculate fire regime attributes for the larger area (`fireRegimePolysCalibration`), `scfmRegime` will estimate fire regime parameters using the larger region, but `scfmDriver` will calibrate spread probability for the smaller region only.
...
rasterToMatchCalibration <- raster(extent(studyAreaCalibration), res = c(250, 250))
crs(rasterToMatchCalibration) <- crs(studyAreaCalibration)
rasterToMatchCalibration[] <- 1
rasterToMatchCalibration <- mask(rasterToMatchCalibration, studyAreaCalibration)
...
# rasterToMatchCalibration <- terra::rast(rasterToMatchCalibration)
